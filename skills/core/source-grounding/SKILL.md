...
Assess independence, proximity to the event, measurement method, incentives, missing population, and contradictory evidence. Multiple pages repeating one upstream claim remain one evidentiary line. For analogies, require independent evidence in both domains, an explicit causal mechanism, a falsifiable implication, and a non-applicability boundary. When resemblance fails, reject a candidate under admission with `kill`; otherwise record `no-action` only if the investigated state stays unchanged.
